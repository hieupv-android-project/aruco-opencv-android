...
// ⚠️ ĐÃ SỬA: Đảo ngược trục Y trong mô hình 3D để khớp với trục Y (hướng xuống) của ảnh OpenCV
void createMarkerModelPoints(double length) {
    float half_length = (float)length / 2.0f;
    marker_model_points.clear();

    // Tọa độ 2D của marker: 0(TL) -> 1(TR) -> 2(BR) -> 3(BL)
    // 0: top-left (trên bên trái)
    marker_model_points.push_back(cv::Point3f(-half_length, half_length, 0));
    // 1: top-right (trên bên phải)
    marker_model_points.push_back(cv::Point3f(half_length, half_length, 0));
    // 2: bottom-right (dưới bên phải)
    marker_model_points.push_back(cv::Point3f(half_length, -half_length, 0));
    // 3: bottom-left (dưới bên trái)
    marker_model_points.push_back(cv::Point3f(-half_length, -half_length, 0));
}
...
Java_com_tuielectronics_aruco_1demo_CameraProcess_setOverlayImage(
    ...
    if (overlayImg.empty()) {
        ALOG("LỖI: setOverlayImage - Không thể giải mã ảnh overlay.");
    } else {
...
Java_com_tuielectronics_aruco_1demo_CameraProcess_setCameraParameters(
    ...
    initArucoDetector(); // Khởi tạo detector
...
Java_com_tuielectronics_aruco_1demo_CameraProcess_openCVProcess(
    ...
    if (pNV21FrameData != nullptr) {
        ...
        for (size_t i = 0; i < makers_detected.size(); ++i) {
            ...
            // ----------------------------------------------------
            // GẮN ẢNH OVERLAY 2D BẰNG HOMOGRAPHY
            // ----------------------------------------------------
            if (!overlayImg.empty()) {
            ...
            if (!cameraMatrix.empty() && marker_model_points.size() == 4) {
                cv::solvePnP(marker_model_points, image_points, cameraMatrix, distCoeffs, rvec_mat, tvec_mat, false, cv::SOLVEPNP_ITERATIVE);
            }
            ...
            if (!rvec_mat.empty() && rvec_mat.depth() == CV_64F) {
                ...
                // Vẽ trục tọa độ thủ công (Đã kiểm tra lại định nghĩa)
